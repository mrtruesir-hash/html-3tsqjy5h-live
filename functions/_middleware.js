...
import VALID_TOOLS_SLUGS from '../valid-tools-slugs.json';
import VALID_RESEARCH_SLUGS from '../valid-research-slugs.json';

const VALID = new Set(VALID_SLUGS);
const VALID_GUIDES = new Set(VALID_GUIDE_SLUGS);
const VALID_TOOLS = new Set(VALID_TOOLS_SLUGS);
const VALID_RESEARCH = new Set(VALID_RESEARCH_SLUGS);
...
const PAGES = new Set(['about', 'affiliate-disclosure', 'app', 'authors', 'contact', 'cookies', 'dbbet-affiliate-program', 'editorial-policy', 'guides', 'instruction', 'mob-cash', 'partner-benefits', 'privacy', 'research', 'responsible-gambling', 'terms', 'tools']);
...
export async function onRequest(context) {
  try {
    ...
    // 5c. soft-404 fix: a /tools/<slug>/ that is not a real published tool -> 404 (Part 5,
    //     mirrors rule 5b; English-only, no lang-prefixed /tools/ tree exists).
    const tm = path.match(/^\/tools\/([^/]+)\/$/);
    if (tm && !VALID_TOOLS.has(tm[1])) {
      return new Response('Not Found', { status: 404, headers: { 'content-type': 'text/plain' } });
    }

    // 5d. soft-404 fix: a /research/<slug>/ that is not a real published research page -> 404
    //     (Part 5, mirrors rule 5b; English-only, no lang-prefixed /research/ tree exists).
    const rm = path.match(/^\/research\/([^/]+)\/$/);
    if (rm && !VALID_RESEARCH.has(rm[1])) {
      return new Response('Not Found', { status: 404, headers: { 'content-type': 'text/plain' } });
    }
    ...
    //    lang-prefixed blog exists), /guides/* /tools/* /research/* (English only, each gated by its
    //    own rule above), or a page that actually exists for that prefix (LANG_PAGES under a
    //    language, the full PAGES set at English root). B1 fix, 2026-08-18: previously this checked
    //    lang-prefixed paths against the full PAGES set, so e.g. /ru/privacy/ (privacy has no /ru/
    //    file) fell through to rule 7 and Cloudflare's static layer soft-served the homepage.
    {
      ...
      const known =
        ...
        (!hasLangPrefix && seg[0] === 'tools') ||              // /tools/ and /tools/<slug>/ — English only
        (!hasLangPrefix && seg[0] === 'research') ||           // /research/ and /research/<slug>/ — English only
