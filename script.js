...
function setLang(flag, name, lang) { var cur = document.querySelector('.lang-current'); if (cur) { cur.querySelector('.flag').textContent = flag; cur.querySelector('.lang-name').textContent = name; } var m = document.getElementById('langMenu'); if (m) { m.classList.remove('open'); } var code = lang || 'en'; var LANGS = ['ru','bn','ar','uz','tr','fr','si','so','sw','fa','az','ur','hi','ne','es','pt']; var parts = location.pathname.replace(/^\/+/, '').split('/'); if (parts[0] && LANGS.indexOf(parts[0]) !== -1) { parts.shift(); } var slug = parts.filter(function (p) { return p; }).join('/'); var prefix = (code === 'en') ? '' : ('/' + code); var target = slug ? (prefix + '/' + slug) : (prefix + '/'); if (location.pathname.replace(/\/+$/, '') !== target.replace(/\/+$/, '')) { window.location.href = target; } }
